...
tags:
  ...
  - "导读"
  - "学习路径"
description: "机器学习理论这 17 篇是对一本 Understanding ML 类教材的逐章精读，本是天然学习路径，缺的只是「章与章之间在升级什么」的导读。本页补上骨架：第二章在有限类 + Realizable 下证明 ERM 收敛（基线）→ 第三章升级为 PAC / Agnostic（去掉 Realizabi……"
---

# 机器学习理论精读路径总览

> 机器学习理论这 17 篇是对一本 Understanding ML 类教材的逐章精读，本是天然学习路径，缺的只是「章与章之间在升级什么」的导读。本页补上骨架：第二章在有限类 + Realizable 下证明 ERM 收敛（基线）→ 第三章升级为 PAC / Agnostic（去掉 Realizability）→ 第四章用 Uniform Convergence 把样本复杂度收束成可计算的界，并标出三篇习题章为可选附录。
