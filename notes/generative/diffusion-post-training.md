...
### 0. 先回顾一下"预训练"阶段在做什么（这是后训练的起点）

**Diffusion（以DDPM为代表）**：定义一个前向加噪过程 $x_0\to x_1\to\dots\to x_T$（逐步变成纯噪声），模型学习反向去噪 $p_\theta(x_{t-1}|x_t)$，训练目标是ELBO（等价于噪声预测的回归损失）。

**Flow Matching / Rectified Flow**：不再显式定义随机加噪过程，而是直接构造从噪声分布到数据分布的一条（通常是直线）插值路径 $x_t=(1-t)x_0+tx_1$，模型学习一个**速度场** $v_\theta(x_t,t)$ 去回归路径的切线方向，采样时通过求解一个ODE（概率流ODE）从噪声积分到数据。SD3、FLUX等新一代文生图/视频模型都是flow matching架构。

两者共同点：**最终生成都是一个"多步骤的、逐渐去噪/演化"的序列过程**——这一点和LLM的"逐token生成"在结构上是同构的，这也是为什么LLM的RLHF/DPO/GRPO几乎都能"翻译"过来。不同点在于：LLM的token采样是**离散、不可微**的；而diffusion的去噪链条、尤其是flow matching的ODE积分，是**连续、可微**的——这个差异会在后面反复出现，是理解很多"扩散模型专属"后训练技巧的关键。
